...
class DQNReplayBuffer:
    ...
    def sample(self):
        ...
        states = torch.from_numpy(np.vstack([np.expand_dims(e.state,0) for e \
        ...
        rewards = torch.from_numpy(np.vstack([e.reward for e \
                                              in experiences if e is not None])).float().to(Param.device)
        next_states = torch.from_numpy(np.vstack([np.expand_dims(e.next_state,0) for e in experiences \
