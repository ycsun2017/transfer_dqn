...
class NewLunarLanderEnv(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(self, obs_type='vel', pos_num=10, noisy_dim=200):
        ...
        if self.obs_type == 'pos':
            self.old_x = np.zeros(pos_num)
            self.old_y = np.zeros(pos_num)
            self.obs_size = 6 + pos_num * 2
        elif self.obs_type == 'noisy':
            self.obs_size = 8 + noisy_dim
            self.noisy_mean = 0
            self.noisy_std = 5
        else:
            self.obs_size = 8
    ...
    def update_pos(self, pos_x, pos_y):
        # print("before", self.old_x, pos_x)
        self.old_x[:-1] = self.old_x[1:]
        self.old_x[-1] = pos_x
        # print("after", self.old_x)
        self.old_y[:-1] = self.old_y[1:]
        self.old_y[-1] = pos_y
    ...
    #             vel.y*(VIEWPORT_H/SCALE/2)/FPS,
                # (self.last_x - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2),
                # (self.last_y - (self.helipad_y+LEG_DOWN/SCALE)) / (VIEWPORT_H/SCALE/2),
                self.lander.angle,
                20.0*self.lander.angularVelocity/FPS,
                1.0 if self.legs[0].ground_contact else 0.0,
                1.0 if self.legs[1].ground_contact else 0.0
                ] + list(self.old_x) + list(self.old_y)
        elif self.obs_type == 'vel':
            state = [
                (pos.x - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2),
                (pos.y - (self.helipad_y+LEG_DOWN/SCALE)) / (VIEWPORT_H/SCALE/2),
                vel.x*(VIEWPORT_W/SCALE/2)/FPS,
...
class NewLunarLanderContinuous(NewLunarLanderEnv):
    continuous = True
